...
test('runtime loads the intact ratified registry through amendment 1.3.2', async () => {
  ...
  assert.equal(bundle.amendmentVersion, '1.3.2');
  ...
  assert.deepEqual(bundle.statusSummary, { enforced: 26, partial: 20, missing: 2 });
...
test('every canonical law has one honest and structurally backed implementation verdict', async () => {
  ...
  for (const law of bundle.laws) {
    ...
    else assert.ok((mapping.gap ?? '').length > 12, `${law.id} exposes its unresolved gap`);
