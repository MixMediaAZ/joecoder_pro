...
function field(name, label, rows = 3) {
  return `<label>${escapeHtml(label)}<textarea name="${name}" rows="${rows}">${escapeHtml(state.brain?.[name] || '')}</textarea></label>`;
}
